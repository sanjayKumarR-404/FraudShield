...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            navigate('/login', { state: { successMsg: "Account created successfully. Please sign in." } });
        } catch (err: any) {
                    ...
                    <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] p-8 rounded-2xl shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-[var(--color-border-subtle)]">
                        <div className="flex justify-between items-center mb-8 border-b border-[var(--color-border)] pb-5">
                            ...
                            <div className="flex bg-[var(--color-bg-elevated)] rounded-lg p-1 border border-[var(--color-border)]">
                                <Link to="/login" className="px-5 py-1.5 text-xs font-semibold text-[var(--color-text-secondary)] hover:text-white transition-all duration-200">Login</Link>
                                <button className="px-5 py-1.5 text-xs font-semibold bg-[var(--color-accent)] text-white rounded shadow-sm transition-all duration-200">Register</button>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div>
                                <label className="block text-xs font-bold uppercase tracking-wider text-[var(--color-text-secondary)] mb-2">Corporate Domain VPA</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-[var(--color-accent)] hover:bg-[#2563eb] active:scale-[0.98] text-white font-semibold rounded-xl p-4 transition-all duration-200 shadow-lg shadow-blue-500/20 disabled:opacity-70 disabled:cursor-wait mt-2 flex justify-center items-center gap-2 uppercase tracking-wide text-xs"
                            >
                                {loading ? (
                                    <>
